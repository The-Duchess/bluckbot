...
# `q add <object> is <description> will add an entry for the key <object>
# `q show <object> will print all entries for the key <object>
...
class Quote < Pluginf
	...
	def script(message, nick, chan)
		message = message[3..-1] #remove `dict from the string
		message.delete! ':`,'
		@r = parse(message)
...
reg_p = /^`q / #regex to call the module
na = "quotes" #name for plugin #same as file name without .rb
de = "`q [add <person> said <quote>] | [show <person>] | [save] | [load]" #description

#plugin = Class_name.new(regex, name, help)
#passed back to the plugins_s array
plugin = Quote.new(reg_p, na, de)
